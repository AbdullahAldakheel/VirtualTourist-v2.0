...
class ImagesViewController: UIViewController, MKMapViewDelegate{
    ...
    @IBAction func editPressed(_ sender: Any) {
        if remove {
            remove = false
            button.setTitle("Press to remove", for: .normal)
...
extension ImagesViewController: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if !remove{
            ...
            let vc = storyboard.instantiateViewController(withIdentifier: "Share") as! ShareImageViewContriller
            let photoToShare = collectionView.cellForItem(at: indexPath) as? PhotoViewCell
                //fetchedResultsController.object(at: indexPath)

            vc.shareImage = photoToShare?.imageView.image
